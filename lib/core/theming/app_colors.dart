import 'package:flutter/material.dart';

class AppColors {
  static const Color mainBlue = Color(0xFF0554cb);
  static const Color blckColor = Color(0xFF000000);
  static const Color whiteColor = Color(0xFFFFFFFF);
  static const Color redColor = Color(0xFFEA1D2C);
  static const Color greenColor = Color(0xFF00B74A);
  static const Color yellowColor = Color(0xFFFFC107);
  // static const Color lightBlue = Color(0xFFF4F8FF);
  // static const Color darkBlue = Color(0xFF242424);
  static const Color gray = Color(0xFF757575);
  static const Color lightGray = Color(0xFFC2C2C2);
  // static const Color lighterGray = Color(0xFFEDEDED);
  // static const Color moreLightGray = Color(0xFFFDFDFF);
  // static const Color moreLighterGray = Color(0xFFF5F5F5);
}
